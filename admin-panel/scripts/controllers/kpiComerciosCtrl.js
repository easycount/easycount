...
angular.module('sbAdminApp')
    .controller('kpiComerciosCtrl', ['$scope', '$timeout', '$http', function ($scope, $timeout, $http) {
        var vm = this;
        vm.tituloPestanya = 0;
        vm.estadisticas = 0;
        vm.prov = "";
        ...
        vm.cargarValoresBar = function(){
            ...
            for (var j=0; j<vm.numMeses; j++){
                var responseValorados = $http.post('./../api/empresas/nuevasEmpresas', {mes: fechaActual.getMonth(), anyo: fechaActual.getFullYear(), provider: vm.prov});
        ...
        vm.cambiarGraficoLineas1 = function() {
            ...
            for(var j=0; j<vm.usuariosPorMes.length; j++){
                ...
                valor.push(vm.usuariosPorMes[j].totalNuevasEmpresas);

                objAux = {
                    mes: vm.dameNombreMes(vm.usuariosPorMes[j].mes)+" del "+vm.usuariosPorMes[j].anyo,
                    nuevos: vm.usuariosPorMes[j].totalNuevasEmpresas
                }
                vm.tituloPestanya= vm.usuariosPorMes[j].totalEmpresas.toString();
                vm.usuariosSistema.push(objAux);
            }

            $scope.line = {
                labels: meses,
                series: ['Nuevos comercios'],
                data: [
                    valor
                ],
                colours: ["#00A731"],
